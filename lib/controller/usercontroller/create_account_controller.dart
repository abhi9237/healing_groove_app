...
class CreateAccountController extends GetxController {
  CreateAccountController({this.arguments, this.comingFrom});
  RxString selectedType = 'user'.obs;
  ...
  void authSelectionCheck() {
    ...
    if (comingFrom == 'login') {
      if (Get.isRegistered<LoginController>()) {
        ...
        emailController.value.text = loginController.emailController.value.text;
        fullNameController.value.text = HiveStorageService.getUserName() ?? '';
        selectedType.value = HiveStorageService.getUserType() ?? '';
      } else {
        var c = Get.put(LoginController());
        emailController.value.text = c.emailController.value.text;
        fullNameController.value.text = HiveStorageService.getUserName() ?? '';
        selectedType.value = HiveStorageService.getUserType() ?? '';
      }
    }
  ...
  void onTapSelectType(String type) {
    ...
    isComingAuthSelection.value = true;
    HiveStorageService.storeUserType(type);
    update();
  }
    ...
    try {
      ...
      Response response = await _authRepository.sendOtp(
        data: {
          ...
          'role': selectedType.value == 'wellness_centre'
              ? 'center_admin'
              : selectedType.value,
        },
      );
    ...
    try {
      ...
      if (response.statusCode == 200) {
        if (context.mounted) {
          ...
          if (verifyOtpResponse.refreshToken != null && verifyOtpResponse.refreshToken!.isNotEmpty) {
            await HiveStorageService.storeRefreshToken(verifyOtpResponse.refreshToken!);
          }
          Get.find<AccountProgressController>().signUpVerified.value = true;

          if (HiveStorageService.getUserType() == 'user') {
            context.push(RouteConstant.tellUsAboutYourself);
          } else if (HiveStorageService.getUserType() == 'wellness_centre') {
            context.push(RouteConstant.setUpYourCentre);
          } else {}
        }
      } else {
        ErrorResponseModel errorResponse = ErrorResponseModel.fromJson(

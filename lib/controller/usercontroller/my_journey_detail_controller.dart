...
class MyJourneyDetailController extends GetxController {
  ...
  MyJourneyRepository myJourneyRepository = MyJourneyRepository();
  ...
  void payBooking(BuildContext context) {
    try {
      ...
      final c = Get.put(PaymentController(context: context));
  ...
  Future<void> getInvoicePdf(BuildContext context) async {
    // _showSnackBar(
    //   context,
    ...
    await genRateInvoice(booking?.id ?? 0);
  }
  ...
  Future<void> genRateInvoice(int bookingId) async {
    ...
    try {
      final response = await myJourneyRepository.genRateInvoice(bookingId);

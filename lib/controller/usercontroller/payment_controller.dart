...
class PaymentController extends GetxController {
  ...
  void _handlePaymentSuccess(PaymentSuccessResponse response) async {
    ...
    await verifyPayment(verifyData, context ?? Get.context!);
  }

  void _handlePaymentError(PaymentFailureResponse response) {
    log(
      'PaymentController: Payment Error: ${response.code} - ${response.message}',
    );
    razorpay.clear();
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        var options = {
          ...
          'description':
              description ??
              Get.find<BookProgramController>().packageModel?.name ??
              '',
          'timeout': 120,
          'prefill': {
  ...
  Future<void> verifyPayment(Map<String, dynamic> data , BuildContext context) async {
    // Show loader dialog
    showDialog(
      context: context,
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        log(
          'PaymentController: Payment Verification Success: ${response.data}',
        );

...
import 'package:healing/presentation/model/response/enquiries_and_bookings_response_model.dart';
...
class EnquiriesAndBookingsController extends GetxController {
  ...
  final EnquiriesAndBookingRepository enquiriesRepository = EnquiriesAndBookingRepository();
  ...
  Future<void> fetchEnquiriesAndBookings({bool isRefresh = false}) async {
    ...
    try {
      ...
      if (response.statusCode == 200 && response.data != null) {
        log('EnquiriesAndBookingsController: response data ===> ${response.data}');
        final responseModel = EnquiriesAndBookingsResponseModel.fromJson(response.data);
  ...
  List<DocModel> get filteredEnquiries {
    return allEnquiries.where((enquiry) {
      final idStr = enquiry.id?.toString() ?? '';
      final titleStr = enquiry.center?.name ?? enquiry.title ?? enquiry.name ?? '';
      ...
      final matchesStatus = _selectedStatus == 'All Status' ||
          (enquiry.status ?? '').toLowerCase() == _selectedStatus.toLowerCase();
